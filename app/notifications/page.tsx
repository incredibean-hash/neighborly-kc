...
export default function NotificationsPage(){
 ...
 return <main className="min-h-screen bg-[#f0f6ff] text-[#00205a]"><header className="bg-[#004687] text-white p-4"><div className="max-w-2xl mx-auto"><Link href="/" className="text-xs opacity-70">← Feed</Link><h1 className="font-black text-2xl">Notifications</h1><p className="text-xs opacity-70">Likes, comments, messages and connections</p></div></header><div className="max-w-2xl mx-auto p-4 space-y-2">{loading?<div className="text-center py-16 opacity-50">Loading…</div>:!items.length?<div className="rounded-2xl bg-white p-10 text-center opacity-60">You're all caught up.</div>:items.map(n=><button key={n.id} onClick={()=>markRead(n.id)} className={`w-full text-left rounded-2xl p-4 border border-[#c2d5f0] ${n.read_at?'bg-white':'bg-[#e6eefb]'}`}><div className="flex gap-3"><span className="text-xl">{n.type==='message'?'💬':n.type==='comment'?'🗨️':n.type==='connection'?'👥':'❤️'}</span><div className="flex-1"><p className="font-medium">{n.message}</p><small className="opacity-40">{new Date(n.created_at).toLocaleString()}</small></div>{!n.read_at&&<span className="w-2 h-2 rounded-full bg-[#004687] mt-2"/>}</div></button>)}</div></main>;
}
