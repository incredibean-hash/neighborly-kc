...
export async function POST(req: NextRequest){
  try{
    ...
    await supabase.from('bluetooth_approvals').insert({
      token, owner, requester, address: address||`${street} ${zip}`, street, zip, status: 'pending', created_at: new Date().toISOString()
    } as any);
    ...
    try{
      await supabase.from('dms').insert({
        ...
        to_user: owner,
        message: `📲 BLUETOOTH TAP REQUEST: ${requester} wants "${address}". Approve: ${approveLink}`,
        body: `Bluetooth approval`
      } as any);
