...
export default function MobileBottomNav({theme,active=''}:{theme:any;active?:string}){
  ...
  return <nav className="nkc-mobile-actions nkc-mobile-bottom-nav" data-theme={theme.id} aria-label="Mobile navigation" style={{backgroundColor:theme.header,color:inactive,borderColor:theme.border,'--nkc-bottom-inactive':inactive} as any}>
    {active==='messages'
      ? <Link href="/" aria-label="Back to Feed" title="Feed" className="nkc-bottom-nav-orb-item" style={orb}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m3 10 9-7 9 7v10a1 1 0 0 1-1 1h-5v-6H9v6H4a1 1 0 0 1-1-1z" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/></svg><span className="sr-only">Back to Feed</span></Link>
      : <Link href="/dms" aria-label="Messages" title="Messages" className="nkc-bottom-nav-orb-item" style={orb}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5h16v11H9l-5 3v-14Z" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/><path d="M8 9h8M8 12.5h5" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"/></svg><span className="sr-only">Messages</span></Link>}
