...
function DmsContent(){
 ...
 const loadMessages=async(id:string)=>{if(!me)return;setSelected(id);const {data,error}=await supabase.from('dms').select('*').or(`and(from_user_id.eq.${me.id},to_user_id.eq.${id}),and(from_user_id.eq.${id},to_user_id.eq.${me.id})`).order('created_at',{ascending:true});if(error)alert(error.message);setMessages(data||[])};
 useEffect(()=>{load()},[]);
 ...
 return <main className="min-h-screen" style={{backgroundColor:theme.bg,color:theme.text}}>
  <header className="p-4" style={{backgroundColor:theme.header,color:theme.headerText}}><div className="max-w-4xl mx-auto flex justify-between items-center gap-3"><div><Link href="/" className="text-xs opacity-70">← Feed</Link><h1 className="font-black text-2xl">Messages</h1></div><Link href="/people" className="shrink-0 rounded-full px-4 py-2 text-sm font-bold" style={{backgroundColor:theme.card,color:theme.text,border:`1px solid ${theme.border}`}}>Find People</Link></div></header>
