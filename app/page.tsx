...
const DEMO_POSTS=[
  {id:'demo1', author_name:'Sarah M', area:'Brookside Area • 5 miles', category:'For Sale', body:'Garage sale Sat 8am - 45th & Wornall, lots of baby stuff + furniture!', created_at:new Date().toISOString()},
  {id:'demo2', author_name:'Mike T', area:'North KC Area • 10 miles', category:'Safety', body:'Heads up - coyote spotted near 64th & N Oak last night, keep pets in', created_at:new Date().toISOString()},
  {id:'demo3', author_name:'KC Parks', area:'Plaza Area • 25 miles', category:'General', body:'Free concert in Mill Creek Park this Friday 6pm! Bring blankets 🎵', created_at:new Date().toISOString()},
];

export default function Page(){
  const [profile,setProfile]=useState<any>(null);
  const [posts,setPosts]=useState<any[]>(DEMO_POSTS); // START with demo posts so you ALWAYS see something
  ...
  async function load(){
    try{
      ...
      if(error){ console.log('DB error, showing demo posts', error); return; }
      if(data && data.length>0){
        // MERGE real posts + demo posts so you always see old AND new
        setPosts([...data,...DEMO_POSTS]);
      }
    }catch(e){ console.log('Load failed, showing demo', e); }
  }
  ...
  async function doPost(){
    if(!text&&!photo){ alert('Write something or add a photo!'); return; }
    const newPost={
      author_name:profile?.full_name||'You',
      zip:profile?.zip||'64155',
      area:profile? profile.zip+' Area' : 'KC Area',
      body:text, text:text, category:cat, reach:reach,
      image_url:photo, photo_url:photo
    };
    // Show instantly
    setPosts([ {...newPost, id:'temp-'+Date.now(), created_at:new Date().toISOString()} as any,...posts ]);
    setText(''); setPhoto(null); setShowPost(false);
    // Try save to DB
    try{ await supabase.from('posts').insert(newPost); load(); }catch(e){ console.log('Saved locally, DB failed', e); }
  }
  ...
  async function del(id:string){
    if(String(id).startsWith('demo')){ setPosts(posts.filter(p=>p.id!==id)); return; }
    await supabase.from('posts').delete().eq('id',id); load();
  }
  const isAdmin=true; // make delete always work for you
  ...
  return(
    <div className={`min-h-screen ${t.bg} ${t.text} pb-28 transition-all duration-500`}>
      ...
      {newDmToast&&<div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-[#ff3b30] text-white px-6 py-3 rounded-full font-bold shadow-2xl text-sm">{newDmToast}</div>}
      ...
      {/* POSTS FEED - GUARANTEED VISIBLE */}
      <div className="max-w-md mx-auto p-3 space-y-3 w-full min-h-[60vh]">
        <div className="text-xs opacity-50 text-center py-2">Showing {shown.length} posts • {filter} • {feedRadius}</div>
        {shown.length===0? (
          <div className={`${t.card} border border-white/10 rounded-2xl p-8 text-center`}>
            <div className="text-3xl mb-2">📭</div>
            <div className="font-bold">No posts in {filter}</div>
            <div className="text-sm opacity-60 mt-1">Try All or post one!</div>
            <button onClick={()=>setFilter('All')} className={`mt-4 ${t.btn} px-6 py-2 rounded-full font-bold text-sm`}>Show All</button>
          </div>
        ) : shown.map((p:any, idx:number)=>(
          <div key={p.id||idx} className={`${t.card} border border-white/10 rounded-2xl p-4 w-full`}>
            <div className="flex justify-between gap-2">
              <div className="min-w-0 flex-1">
                <div className="font-bold text-[15px] truncate">{p.author_name} {String(p.id).startsWith('demo')&&<span className="text-[10px] opacity-40">• demo</span>}</div>
            ...
            <button onClick={()=>{setDmTo(p.author_name); setShowDM(true); loadDMs();}} className={`mt-3 w-full ${t.btn} py-3 rounded-full font-black text-sm`}>DM {p.author_name.split(' ')[0]}</button>
          </div>
        ))}
      </div>
      ...
      <div className={`fixed bottom-0 left-0 right-0 ${t.bg} border-t border-white/20 flex justify-around items-center py-3 max-w-md mx-auto z-20 shadow-2xl`}>
        ...
        <button onClick={()=>setShowPost(true)} className={`${t.btn} w-14 h-14 rounded-full font-black text-2xl shadow-xl hover:scale-105 transition-transform flex items-center justify-center`}>+</button>
      ...
      {showPost&&<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-30 flex items-end sm:items-center justify-center p-0 sm:p-4"><div className={`${t.card} w-full max-w-sm rounded-t-[2rem] sm:rounded-2xl p-6 border border-white/10 ${t.text} shadow-2xl`}>
        <h2 className="font-bold text-lg text-center mb-4">New Post — always visible!</h2>
        <div className="flex flex-wrap gap-2 mb-4 justify-center">{CATS.map(c=><button key={c} onClick={()=>setCat(c)} className={`px-5 py-2 rounded-full text-sm font-black ${cat===c? t.btn : t.sub}`}>{c}</button>)}</div>
        ...
        <textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Whats happening in KC? This will show instantly!" className={`w-full ${t.bg} border-2 border-white/20 rounded-xl p-4 text-base min-h-[120px] ${t.text} w-full focus:border-white/40`} autoFocus/>
        <div className="flex items-center gap-3 mt-4">
          ...
          <span className="text-xs opacity-60">Add photo — shows right away</span>
        ...
        <div className="flex gap-3 mt-6"><button onClick={()=>setShowPost(false)} className={`flex-1 py-3 rounded-full ${t.sub} font-bold`}>Cancel</button><button onClick={doPost} className={`flex-1 py-3 rounded-full ${t.btn} font-black text-base`}>Post Now →</button></div>
      </div></div>}
      ...
      {showDM&&<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-20 flex items-end sm:items-center justify-center p-0 sm:p-4"><div className={`${t.card} w-full max-w-sm rounded-t-[2rem] sm:rounded-2xl p-6 border border-white/10 max-h-[85vh] overflow-auto ${t.text} shadow-2xl w-full`}>
        ...
        <div className="space-y-2">{dms.map((m:any)=><div key={m.id} onClick={()=>{setDmTo(m.from_user===profile?.full_name? m.to_user : m.from_user);}} className={`${t.sub} p-3 rounded-xl text-sm cursor-pointer break-words`}><b>{m.from_user} → {m.to_user}</b><div className="mt-1 opacity-80 break-words">{m.message||m.body}</div></div>)}</div>
