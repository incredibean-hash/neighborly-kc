...
export default function Page(){
  ...
  useEffect(()=>{
    const savedTheme = localStorage.getItem('nkc_theme'); if(savedTheme && THEMES[savedTheme]) setThemeId(savedTheme);
    (async()=>{
    ...
    <div className="min-h-screen" style={{backgroundColor: theme.bg, color: theme.text}}>
      <header className="sticky top-0 z-40 border-b" style={{backgroundColor: theme.header, borderColor: theme.border}}>
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
          <div><h1 className="font-black text-xl tracking-tight" style={{color: theme.id==='royals' || theme.id==='daylight'? '#fff' : theme.id==='pip-boy'? theme.text : '#fff'}}>Neighborly KC</h1><p className="text-xs -mt-1" style={{color: theme.id==='royals' || theme.id==='daylight'? '#ffffff99' : theme.subtext}}>Kansas City • 40 Mile Radius</p></div>
          <div className="flex items-center gap-2">
            ...
            {profile? <><span className="text-xs hidden sm:block" style={{color: theme.subtext}}>{profile.full_name}</span><button onClick={()=>{localStorage.removeItem('nkc_profile'); supabase.auth.signOut(); setProfile(null);}} className="px-3 py-1.5 rounded-full text-xs font-bold" style={{backgroundColor: theme.card, color: theme.text, border: `1px solid ${theme.border}`}}>Sign out</button></> : <button onClick={()=>setShowJoin(true)} className="px-4 py-2 rounded-full text-sm font-black" style={{backgroundColor: theme.pillActive, color: theme.pillTextActive}}>Join / Sign in</button>}
          </div>
        <div className="max-w-6xl mx-auto px-6 pb-3 flex gap-2 justify-center flex-wrap">
          {['Feed','Safety','For Sale'].map(t=>{ const active = (cat==='All' && t==='Feed') || (t==='Safety' && cat==='Safety Alert') || (t==='For Sale' && cat==='For Sale & Free'); return <button key={t} onClick={()=>{ if(t==='Feed') setCat('All'); if(t==='Safety') setCat('Safety Alert'); if(t==='For Sale') setCat('For Sale & Free'); }} className="px-4 py-1.5 rounded-full text-sm font-bold transition" style={{backgroundColor: active? theme.pillActive : theme.pillInactive, color: active? theme.pillTextActive : theme.text, border: `1px solid ${theme.border}`}}>{t}</button> })}
        </div>
      </header>

      <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-[220px_1fr_300px] gap-6">
        <aside className="rounded-2xl p-3 h-fit border hidden lg:block" style={{backgroundColor: theme.card, borderColor: theme.border}}><p className="text-xs font-bold px-3 py-2" style={{color: theme.subtext}}>FILTER</p>{CATS.map(c=><button key={c} onClick={()=>setCat(c)} className="w-full text-left px-3 py-2.5 rounded-xl text-sm transition" style={{backgroundColor: cat===c? theme.accent : 'transparent', color: cat===c? theme.pillTextActive : theme.text}}>{c}</button>)}</aside>

        <main className="space-y-3">
          <div className="rounded-2xl p-4 border" style={{backgroundColor: theme.card, borderColor: theme.border}}><textarea value={body} onChange={e=>setBody(e.target.value)} placeholder={profile?`What's up in ${cur?.name}?`:'Join Parkwood Hills to post...'} className="w-full rounded-xl p-3 min-h-[80px] text-sm outline-none" style={{backgroundColor: theme.input, color: theme.text, border: `1px solid ${theme.border}`}} /><div className="flex items-center gap-2 mt-3"><input id="file-input" type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>setFile(e.target.files?.[0]||null)} className="text-xs" style={{color: theme.subtext}} />{file && <span className="text-xs" style={{color: theme.subtext}}>{(file.size/1024).toFixed(0)}KB → ~400KB</span>}</div><div className="flex justify-end mt-2"><button disabled={uploading} onClick={handlePost} className="px-5 py-2 rounded-full text-sm font-bold disabled:opacity-50" style={{backgroundColor: theme.accent, color: theme.pillTextActive}}>{uploading?'Uploading...':'Post to neighbors'}</button></div></div>

          <div className="rounded-2xl p-4 border overflow-hidden" style={{backgroundColor: theme.card, borderColor: theme.border}}>
            <div className="flex justify-between items-start"><div><p className="font-bold text-sm">You</p><p className="text-xs" style={{color: theme.subtext}}>Feed • KC Area</p><p className="text-sm mt-2 font-bold">Here</p></div><button className="w-6 h-6 rounded-full flex items-center justify-center text-xs" style={{backgroundColor: theme.input}}>✕</button></div>
            <div className="mt-3 rounded-[32px] aspect-square flex items-center justify-center border-8 overflow-hidden" style={{backgroundColor: theme.bg, borderColor: theme.card}}>
              <div className="text-[120px] font-black tracking-tighter" style={{color: theme.text, textShadow: theme.glow || 'none'}}>NK</div>
              <div className="text-[120px] font-black tracking-tighter -ml-6" style={{color: theme.subtext}}>C</div>
            </div>
            <button className="mt-3 w-full py-3 rounded-full font-bold text-sm" style={{backgroundColor: theme.pillActive, color: theme.pillTextActive}}>DM You</button>
          </div>

          {filtered.map((p:any)=>{ const cList=comments[p.id]||[]; const isOpen=openComments[p.id]; const pLikes=likes[p.id]||[]; const liked=pLikes.some((l:any)=>l.author_name===profile?.full_name); const isOwner = profile && (p.profiles?.full_name===profile.full_name || p.author_name===profile.full_name); const canDelete = isOwner || isAdmin; return (<div key={p.id} className="rounded-2xl p-4 border" style={{backgroundColor: theme.card, borderColor: theme.border}}><div className="flex justify-between"><p className="text-xs font-bold" style={{color: theme.subtext}}>{p.profiles?.full_name||p.author_name||'Neighbor'} · {p.category}</p>{canDelete && <button onClick={()=>deletePost(p.id,p.image_url)} className="text-xs opacity-40 hover:text-red-500">🗑️ Delete</button>}</div><p className="mt-1 whitespace-pre-wrap">{p.body || p.content}</p>{p.image_url && <img src={p.image_url} alt="post" className="mt-3 rounded-xl max-h-[400px] w-full object-cover border" style={{borderColor: theme.border}} />}<p className="text-xs mt-2" style={{color: theme.subtext}}>{new Date(p.created_at).toLocaleString()}</p><div className="mt-3 pt-3 border-t flex gap-4" style={{borderColor: theme.border}}><button onClick={()=>togglePostLike(p.id)} className="text-xs font-bold">{liked?'❤️':'🤍'} {pLikes.length}</button><button onClick={()=>setOpenComments((prev)=>({...prev,[p.id]:!prev[p.id]}))} className="text-xs font-bold hover:underline" style={{color: theme.subtext}}>💬 {cList.length} {isOpen?'▲':'▼'}</button></div>{isOpen && (<div className="mt-3 rounded-xl p-3 space-y-2" style={{backgroundColor: theme.input}}>{cList.map((c:any)=>{ const cl=cLikes[c.id]||[]; const cliked=cl.some((l:any)=>l.author_name===profile?.full_name); const canDelC = (profile && c.author_name===profile.full_name) || isAdmin; return (<div key={c.id} className="text-sm rounded-lg p-2 flex justify-between gap-2" style={{backgroundColor: theme.card}}><div><b className="text-xs">{c.author_name}:</b> {c.content||c.body} <span className="text-[10px] ml-2" style={{color: theme.subtext}}>{new Date(c.created_at).toLocaleTimeString()}</span><button onClick={()=>toggleCommentLike(c.id)} className="ml-3 text-xs">{cliked?'❤️':'🤍'} {cl.length}</button></div>{canDelC && <button onClick={()=>deleteComment(c.id,p.id)} className="text-[10px] opacity-30 hover:text-red-500">🗑️</button>}</div>);})}<div className="flex gap-2 pt-2"><input value={commentText[p.id]||''} onChange={e=>setCommentText((prev)=>({...prev,[p.id]:e.target.value}))} placeholder={profile?'Add a comment...':'Join to comment'} className="flex-1 border rounded-full px-3 py-2 text-sm outline-none" style={{backgroundColor: theme.card, borderColor: theme.border, color: theme.text}} /><button onClick={()=>addComment(p.id)} className="px-4 py-2 rounded-full text-xs font-bold" style={{backgroundColor: theme.accent, color: theme.pillTextActive}}>Reply</button></div></div>)}</div>)})}
        </main>
        <aside className="rounded-2xl p-5 border h-fit" style={{backgroundColor: theme.card, borderColor: theme.border}}><h3 className="font-black">{cur?.name}</h3><p className="text-xs" style={{color: theme.subtext}}>{cur?.zip} · Kansas City, MO</p><div className="grid grid-cols-2 gap-2 mt-4"><div className="rounded-xl p-3 text-center" style={{backgroundColor: theme.input}}><b className="text-lg">{cur?.member_count}</b><p className="text-xs">NEIGHBORS</p></div><div className="rounded-xl p-3 text-center" style={{backgroundColor: theme.input}}><b className="text-lg">{posts.length}</b><p className="text-xs">POSTS</p></div></div></aside>
      </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="rounded-[24px] w-full max-w-sm p-5 border max-h-[80vh] overflow-y-auto" style={{backgroundColor: '#15181f', borderColor: '#262a33'}}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.values(THEMES).map((t:any)=>{ const active = themeId===t.id; return (
                <button key={t.id} onClick={()=>setTheme(t.id)} className="rounded-2xl p-3 text-left border-2 text-sm font-bold transition-all flex flex-col gap-2" style={{backgroundColor: t.card, borderColor: active? '#fff' : t.border, color: t.text, boxShadow: active? '0 0 0 2px #fff' : 'none'}}>
                  <div className="w-6 h-6 rounded-full border-2 flex items-center justify-center" style={{borderColor: t.text, backgroundColor: active? t.text : 'transparent'}}>{active && <div className="w-2 h-2 rounded-full" style={{backgroundColor: t.id==='pip-boy'? '#000' : t.card}}></div>}</div>
      ...
      {showJoin && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="rounded-[28px] w-full max-w-sm p-6 shadow-2xl border" style={{backgroundColor: theme.card, borderColor: theme.border, boxShadow: theme.glow || '0 25px 50px -12px rgba(0,0,0,0.5)'}}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full flex items-center justify-center font-black" style={{backgroundColor: theme.accent, color: theme.pillTextActive}}>NK</div>
              <div><h2 className="font-black text-lg leading-none">Join {cur?.name}</h2><p className="text-[11px] font-bold mt-1" style={{color: theme.subtext}}>{theme.id==='pip-boy'? 'VAULT-TEC SECURE LINK • 64155' : theme.id==='chiefs'? 'ARROWHEAD SECURE LINK • 64155' : theme.id==='royals'? 'THE K • 64155' : theme.id==='sporting'? "CHILDREN'S MERCY PARK • KC" : 'KANSAS CITY • 40 MILE RADIUS • PRIVATE'}</p></div>
            </div>

            {theme.id==='pip-boy' && <div className="mt-4 font-mono text-[10px] opacity-70" style={{color: theme.text}}>&gt; INITIALIZING SECURE CONNECTION...<br/>&gt; ENCRYPTION: VAULT-TEC STANDARD<br/>&gt; STATUS: AWAITING AUTHENTICATION</div>}

            <button onClick={signInWithGoogle} disabled={googleLoading} className="mt-5 w-full border-2 py-3.5 rounded-full font-black text-sm flex items-center justify-center gap-2.5 transition-all hover:scale-[1.02] active:scale-[0.98]" style={{backgroundColor: theme.id==='royals' || theme.id==='daylight'? '#fff' : theme.card, borderColor: theme.id==='pip-boy' || theme.id==='chiefs'? theme.accent : '#000', color: theme.id==='pip-boy' || theme.id==='chiefs' || theme.id==='sporting'? theme.accent : '#000'}}>
            ...
            <div className="flex items-center gap-3 my-5"><div className="h-px flex-1" style={{backgroundColor: theme.border}}></div><span className="text-[10px] font-black tracking-widest" style={{color: theme.subtext}}>{theme.id==='pip-boy'? '// OR USE EMAIL //' : 'OR JOIN WITH EMAIL'}</span><div className="h-px flex-1" style={{backgroundColor: theme.border}}></div></div>
            ...
            <form onSubmit={e=>{e.preventDefault(); const pr={full_name:name,email,street_address:addr,zip:cur?.zip,neighborhood_id:cur?.id}; localStorage.setItem('nkc_profile',JSON.stringify(pr)); setProfile(pr); setShowJoin(false);}} className="space-y-3">
              <input required value={name} onChange={e=>setName(e.target.value)} placeholder="Full name" className="w-full rounded-xl px-4 py-3.5 text-sm outline-none focus:ring-2" style={{backgroundColor: theme.input, border: `1px solid ${theme.border}`, color: theme.text}}/>
              <input required value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email" className="w-full rounded-xl px-4 py-3.5 text-sm outline-none focus:ring-2" style={{backgroundColor: theme.input, border: `1px solid ${theme.border}`, color: theme.text}}/>
              <input required value={addr} onChange={e=>setAddr(e.target.value)} placeholder={`Address in ${cur?.zip}`} className="w-full rounded-xl px-4 py-3.5 text-sm outline-none focus:ring-2" style={{backgroundColor: theme.input, border: `1px solid ${theme.border}`, color: theme.text}}/>
              <div className="flex gap-2 pt-2"><button type="button" onClick={()=>setShowJoin(false)} className="flex-1 py-3.5 rounded-full font-bold text-sm border" style={{backgroundColor: theme.input, borderColor: theme.border, color: theme.text}}>Cancel</button><button className="flex-1 py-3.5 rounded-full font-black text-sm" style={{backgroundColor: theme.accent, color: theme.pillTextActive}}>{theme.id==='pip-boy'? 'ENTER VAULT' : `Join ${cur?.name.split(' ')[0]}`}</button></div>
            </form>
            <p className="text-[10px] text-center mt-4 font-mono" style={{color: theme.subtext}}>{theme.id==='pip-boy'? 'ROBCO INDUSTRIES // VAULT-TEC APPROVED' : theme.id==='royals'? 'ROYALS • KANSAS CITY • THE K' : 'Secure via Supabase Auth • 40 mile KC radius'}</p>
          </div>
        </div>
      )}
    </div>
