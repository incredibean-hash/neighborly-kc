...
const PipBoyIcon=({name,alt='',className=''}:{name:string;alt?:string;className?:string})=>
  <img src={`/pipboy/${name}.png`} alt={alt} className={`nkc-pipboy-icon ${className}`} draggable={false}/>;
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden nkc-app-shell" style={{backgroundColor: theme.bg, color: theme.text, colorScheme: theme.id==='aim' ? 'light' : 'dark'}}>
              ...
              <div className="flex justify-between gap-3"><div className="flex items-center gap-2 min-w-0"><div className="w-9 h-9 shrink-0 rounded-full overflow-hidden grid place-items-center font-black text-xs border" style={{backgroundColor:theme.input,borderColor:theme.border}}>{p.profiles?.avatar_url?<img src={p.profiles.avatar_url} alt="" className="w-full h-full object-cover"/>:(p.profiles?.full_name||p.author_name||'N').slice(0,1).toUpperCase()}</div><div><div className="flex items-center gap-1.5 flex-wrap"><p className="text-xs font-bold opacity-60">{(p.user_id||p.author_id)?<a href={`/dms?user=${p.user_id||p.author_id}`} className="hover:underline">{p.profiles?.full_name||p.author_name||'Neighbor'}</a>:(p.profiles?.full_name||p.author_name||'Neighbor')} · {p.category}</p>
                ...
                {canManage&&<details className="nkc-admin-menu relative shrink-0">
                  ...
                  <div className="nkc-admin-menu-panel" style={{backgroundColor:theme.card,borderColor:theme.border,color:theme.text}}>
                    <button type="button" onClick={(event)=>{event.currentTarget.closest('details')?.removeAttribute('open');beginEdit(p)}}>{theme.id==='pip-boy'&&<PipBoyIcon name="edit" alt="" className="nkc-pipboy-menu-icon"/>}✏️ Edit post</button>
                    <button type="button" onClick={()=>deletePost(p.id,p.image_url,p)}>{theme.id==='pip-boy'&&<PipBoyIcon name="delete" alt="" className="nkc-pipboy-menu-icon"/>}🗑️ Remove post</button>
                    {isAdmin&&<>
                      <button type="button" onClick={()=>viewPostReports(p)}>{theme.id==='pip-boy'&&<PipBoyIcon name="report" alt="" className="nkc-pipboy-menu-icon"/>}🚩 View reports</button>
                      ...
                      <button type="button" onClick={()=>togglePostModeration(p,'is_pinned',p.is_pinned?'Unpin post':'Pin post')}>{theme.id==='pip-boy'&&<PipBoyIcon name="pin" alt="" className="nkc-pipboy-menu-icon"/>}{p.is_pinned?'📌 Unpin post':'📌 Pin post'}</button>
                      {!isOwner&&<>
      ...
      >
        ...
        >
          {theme.id==='pip-boy'?<PipBoyIcon name="dms" alt="" className="nkc-pipboy-nav-icon"/>:<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5h16v11H9l-5 3v-14Z" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/><path d="M8 9h8M8 12.5h5" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"/></svg>}
        ...
        <button
          ...
          style={{color:navThemeColor}}
        >
          {theme.id==='pip-boy'?<PipBoyIcon name="plus" alt="" className="nkc-pipboy-plus-icon"/>:<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"/></svg>}
        </button>
        ...
        >
          {theme.id==='pip-boy'?<PipBoyIcon name="settings" alt="" className="nkc-pipboy-nav-icon"/>:<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9.7 3.6h4.6l.6 2.1 1.8 1 2.1-.5 2.3 4-1.5 1.6v2.1l1.5 1.6-2.3 4-2.1-.5-1.8 1-.6 2.1H9.7L9.1 20l-1.8-1-2.1.5-2.3-4 1.5-1.6v-2.1L2.9 10l2.3-4 2.1.5 1.8-1z" fill="none" stroke="currentColor" strokeWidth="1.55" strokeLinejoin="round"/><circle cx="12" cy="12.8" r="2.7" fill="none" stroke="currentColor" strokeWidth="1.7"/></svg>}
        ...
        <div className="nkc-create-post-screen nkc-pop-in" style={{backgroundColor:theme.bg,color:theme.text}} role="dialog" aria-modal="true" aria-labelledby="create-post-title">
          <header className="nkc-create-post-header" style={{backgroundColor:theme.header,borderColor:theme.border}}>
            ...
            <div className="min-w-0 text-center">
              {theme.id==='pip-boy'&&<PipBoyIcon name="create-post" alt="" className="nkc-pipboy-create-header"/>}
