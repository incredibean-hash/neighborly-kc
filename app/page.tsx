...
export default function Page(){
  ...
  const togglePostLike = async (postId:string) => {
    ...
    if(myLike){
      ...
      setLikes((prev)=>{
        const next = {...prev};
        next[postId]=prev[postId].filter((x:any)=>x.id!==myLike.id);
        return next;
      });
    } else {
  ...
  const toggleCommentLike = async (cId:string) => {
    ...
    if(myLike){
      ...
      setCLikes((prev)=>{
        const next = {...prev};
        next[cId]=prev[cId].filter((x:any)=>x.id!==myLike.id);
        return next;
      });
    } else {
  ...
  const deleteComment = async (cId:string, postId:string) => {
    ...
    setComments((prev)=>{
      const next = {...prev};
      next[postId]=prev[postId].filter((x:any)=>x.id!==cId);
      return next;
    });
  };
          ...
          {filtered.map((p:any)=>{
            ...
            <div key={p.id} className="bg-white rounded-2xl p-4 border">
              ...
              <div className="mt-3 pt-3 border-t flex gap-4">
                ...
                <button onClick={()=>setOpenComments((prev)=>({...prev,[p.id]:!prev[p.id]}))} className="text-xs font-bold hover:underline opacity-60">💬 {cList.length} Comment{cList.length!==1?'s':''} {isOpen?'▲':'▼'}</button>
              </div>
              {isOpen && (
                <div className="mt-3 bg-[#f8f5ee] rounded-xl p-3 space-y-2">
                  ...
                  <div className="flex gap-2 pt-2"><input value={commentText[p.id]||''} onChange={e=>setCommentText((prev)=>({...prev,[p.id]:e.target.value}))} placeholder={profile?'Add a comment...':'Join to comment'} className="flex-1 bg-white border rounded-full px-3 py-2 text-sm outline-none" /><button onClick={()=>addComment(p.id)} className="bg-[#1a3a2f] text-white px-4 py-2 rounded-full text-xs font-bold">Reply</button></div>
                </div>
              )}
            </div>
          )})}
