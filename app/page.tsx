    ...
    <div className="min-h-screen bg-[#f8f5ee]">
      <header className="bg-white border-b sticky top-0 z-20 px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex gap-2 items-center">
          {profile ? (
            <>
              <span className="text-xs hidden sm:block">{profile.full_name} - {profile.street_address}</span>
              <Link href="/dms" className="bg-black text-white px-3 py-1.5 rounded-full text-xs font-bold">DMs</Link>
              <button onClick={()=>{ localStorage.removeItem('nkc_profile_tiered_40'); localStorage.removeItem('nkc_profile'); setProfile(null); }} className="bg-gray-100 px-2 py-1 rounded-full text-[10px]">Leave</button>
            </>
          ) : (
            <button onClick={()=>{ console.log('Join clicked'); setShowJoin(true); }} className="bg-[#1a3a2f] text-white px-5 py-2 rounded-full text-sm font-black active:scale-95">Join</button>
          )}
        </div>
      </header>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white rounded-t-[24px] sm:rounded-[20px] w-full max-w-[480px] p-5 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button onClick={()=>{
                if(!name || name.trim().length < 2){ alert('Enter your full name first'); return; }
                try{
                  const pr={full_name:name.trim(),email:email.trim(),street_address:addr.trim()||'304 NE 115TH ST',zip:zip.trim()||cur?.zip||'64155', tier:'5mile', is_verified:false, neighborhood_id:cur?.id, is_founder:false};
                  localStorage.setItem('nkc_profile_tiered_40',JSON.stringify(pr));
                  localStorage.setItem('nkc_profile',JSON.stringify(pr));
                  alert('✅ Joined! 5 Mile unlocked');
                  setShowJoin(false);
                  window.location.reload();
                }catch(e:any){ alert('Join failed: '+e.message); }
              }} className="flex-1 bg-amber-500 py-3 rounded-full font-bold text-sm active:scale-95">Join 5 Mile</button>
              <button disabled={!aiVerified} onClick={()=>{
                if(!aiVerified){ alert('Verify mail first - upload envelope and click Verify'); return; }
                if(!name || name.trim().length < 2){ alert('Enter your full name first'); return; }
                try{
                  const pr={full_name:name.trim(),email:email.trim(),street_address:aiParsedAddress.street,zip:aiParsedAddress.zip, tier:'40mile', is_verified:true, via_mail:true, is_founder:true, neighborhood_id:cur?.id};
                  localStorage.setItem('nkc_profile_tiered_40',JSON.stringify(pr));
                  localStorage.setItem('nkc_profile',JSON.stringify(pr));
                  alert(`✅ ${aiParsedAddress.full} - 40 mile unlocked`);
                  setShowJoin(false);
                  window.location.reload();
                }catch(e:any){ alert('Join failed: '+e.message); }
              }} className="flex-1 py-3 rounded-full font-bold text-sm bg-black text-white disabled:opacity-30 active:scale-95">Join 40 Mile ✅</button>
            </div>
            <button onClick={()=>{ localStorage.removeItem('nkc_profile_tiered_40'); localStorage.removeItem('nkc_profile'); alert('Cleared - reload'); window.location.reload(); }} className="w-full mt-2 text-[10px] opacity-30">Reset / Clear Profile (if stuck)</button>
          </div>
        </div>
      )}
    </div>
