...
const THEMES={
  ...
  daylight:{name:'Daylight', bg:'bg-[#f8f5ee]', card:'bg-white', text:'text-black', sub:'bg-black/5', btn:'bg-black text-white'},
...
export default function Page(){
  ...
  useEffect(()=>{
    ...
    const th=localStorage.getItem('nkc_theme') as any; if(th && THEMES[th]) setTheme(th);
    load();
  },[]);
    ...
    <div className={`min-h-screen ${t.bg} ${t.text} pb-24`}>
      ...
      <div className="max-w-md mx-auto p-4 space-y-4">
        {shown.map((p:any, idx:number)=>(
          <div key={p.id||idx} className={`${t.card} border border-white/10 rounded-2xl p-5`}>
            ...
            <button onClick={()=>{setDmTo(p.author_name); setShowDM(true); loadDMs();}} className={`mt-4 w-full ${t.btn} py-3 rounded-full font-black text-sm`}>DM {p.author_name.split(' ')[0]}</button>
          </div>
        ))}
      </div>

      <div className={`fixed bottom-0 left-0 right-0 ${t.bg} border-t border-white/10 flex justify-around items-center py-4 max-w-md mx-auto`}>
        <button className="text-sm font-bold">Home</button>
        <button onClick={()=>setShowPost(true)} className={`${t.btn} w-14 h-14 rounded-full font-black text-2xl`}>+</button>
      ...
      {showDM&&<div className="fixed inset-0 bg-black/80 z-20 flex items-center justify-center p-4"><div className={`${t.card} w-full max-w-sm rounded-2xl p-6 border border-white/10 max-h-[80vh] overflow-auto ${t.text}`}>
