...
const THEMES:any={
  ...
  daylight:{name:'Daylight ☀️', bg:'bg-[#f8f5ee]', card:'bg-white', text:'text-black', sub:'bg-black/5', btn:'bg-black text-white'},
  kcblue:{name:'KC Blue 💙', bg:'bg-[#0a1931]', card:'bg-[#123456]', text:'text-white', sub:'bg-white/10', btn:'bg-[#ffcc00] text-black'},
  sand:{name:'Warm Sand', bg:'bg-[#e8ddd0]', card:'bg-[#fff8ee]', text:'text-[#2b2b2b]', sub:'bg-black/5', btn:'bg-[#1a3a2f] text-white'},
...
export default function Page(){
  ...
  // Smooth settings open/close
  useEffect(()=>{
    if(showSettings){ setTimeout(()=>setSettingsAnim(true),10); }
    else setSettingsAnim(false);
  },[showSettings]);
  ...
  return(
    <div className={`min-h-screen ${t.bg} ${t.text} pb-24 transition-all duration-500 ease-in-out ${t.accent==='pip'?'font-mono tracking-wider':''}`}>
      <style>{`
        * { transition: background-color 0.4s ease, color 0.4s ease, border-color 0.4s ease; }
        button, select, textarea, input { transition: all 0.2s ease; }
        button:active { transform: scale(0.96); }
      `}</style>

      {t.accent==='pip'&&<div className="pointer-events-none fixed inset-0 bg-[repeating-linear-gradient(0deg,rgba(0,255,65,0.05),rgba(0,255,65,0.05)_1px,transparent_2px)] z-0 transition-opacity duration-700"></div>}

      <div className={`sticky top-0 ${t.bg} border-b ${t.accent==='pip'?'border-[#00ff41]/30':'border-white/10'} p-4 z-10 transition-all duration-500`}>
        <div className="flex justify-between items-center max-w-md mx-auto">
          <div className="text-center flex-1">
            <div className={`text-xl font-black tracking-widest transition-all duration-500 ${t.accent==='aim'?'font-mono bg-[#0055ff] text-white inline-block px-3 py-1 border-2 border-black':''} ${t.accent==='pip'?'text-[#00ff41]':''}`}>
              {t.accent==='aim'?'Neighborly KC - Buddy List': t.accent==='pip'?'ROBCO INDUSTRIES':'Neighborly KC'}
            </div>
            <div className={`text-[11px] mt-1 transition-all duration-500 ${t.accent==='pip'?'text-[#00ff41]/70':'opacity-50'}`}>Kansas City • 40 Mile Radius</div>
          </div>
        ...
        <div className="flex justify-center mt-3">
          <select value={feedRadius} onChange={e=>setFeedRadius(e.target.value)} className={`${t.card} border border-white/10 rounded-full px-4 py-1.5 text-xs ${t.text} transition-all duration-300 focus:ring-2`}>
      ...
      <div className="max-w-md mx-auto p-4 space-y-4 relative z-10">
        {shown.map((p:any, idx:number)=>(
          <div key={p.id||idx} className={`${t.card} border ${t.accent==='pip'?'border-[#00ff41]/30':'border-white/10'} rounded-2xl p-5 transition-all duration-500 hover:shadow-xl hover:-translate-y-1`} style={{animation:`fadeIn 0.5s ease ${idx*0.05}s both`}}>
            <div className="flex justify-between">
              <div>
                <div className="font-bold text-base">{p.author_name}</div>
                <div className="text-xs opacity-50 mt-1 transition-all duration-300">{p.category} • {p.area||'KC Area'} {p.reach? '• '+p.reach : ''}</div>
              </div>
              {(p.id && profile&&(profile.full_name===p.author_name||isAdmin))&&<button onClick={()=>del(p.id)} className="text-xs bg-red-900/60 text-white px-4 py-1 rounded-full h-fit hover:bg-red-800 transition-colors">Delete</button>}
            </div>
            <div className="mt-3 text-[15px] leading-relaxed">{p.body||p.text}</div>
            {(p.image_url||p.photo_url)&&<img src={p.image_url||p.photo_url} className="mt-4 rounded-xl w-full transition-all duration-500 hover:scale-[1.02]"/>}
            <button onClick={()=>{setDmTo(p.author_name); setShowDM(true); loadDMs();}} className={`mt-4 w-full ${t.btn} py-3 rounded-full font-black text-sm transition-all duration-300 hover:shadow-lg hover:scale-[1.02] active:scale-[0.98]`}>DM {p.author_name.split(' ')[0]}</button>
          </div>
        ))}
      </div>

      <div className={`fixed bottom-0 left-0 right-0 ${t.bg} border-t ${t.accent==='pip'?'border-[#00ff41]/30':'border-white/10'} flex justify-around items-center py-4 max-w-md mx-auto z-10 transition-all duration-500 backdrop-blur-lg bg-opacity-90`}>
        <button className="text-sm font-bold hover:scale-110 transition-transform">Home</button>
        <button onClick={()=>setShowPost(true)} className={`${t.btn} w-14 h-14 rounded-full font-black text-2xl shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300`}>+</button>
        <button onClick={()=>{loadDMs();setShowDM(true);}} className="text-sm font-bold hover:scale-110 transition-transform">DM</button>
      </div>
          ...
          <div onClick={e=>e.stopPropagation()} className={`${t.card} w-full max-w-sm rounded-t-[2rem] sm:rounded-2xl p-6 border border-white/10 ${t.text} shadow-2xl transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${settingsAnim?'translate-y-0 opacity-100':'translate-y-full sm:translate-y-8 opacity-0'}`}>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {Object.keys(THEMES).map((k:any, i)=><button key={k} onClick={()=>{setTheme(k);}} className={`p-4 rounded-2xl border-2 text-left transition-all duration-300 hover:scale-[1.03] hover:shadow-lg ${theme===k? 'border-white scale-[1.02] shadow-lg' : 'border-transparent'} ${THEMES[k].card}`} style={{transitionDelay:`${i*30}ms`}}>
                  <div className={`w-8 h-8 rounded-full mb-2 ${THEMES[k].bg} border border-white/20 transition-all duration-300`}></div>
                  <div className="font-bold text-sm">{THEMES[k].name}</div>
                  <div className={`text-[10px] mt-1 transition-all duration-300 ${theme===k?'opacity-100':'opacity-0'}`}>✓ Active</div>
                </button>)}
              </div>
            </div>
            <button onClick={()=>setShowSettings(false)} className={`w-full ${t.btn} py-3.5 rounded-full font-black transition-all duration-300 hover:shadow-lg active:scale-[0.98]`}>Done</button>
          </div>
        </div>
      )}
      ...
      {showPost&&<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-20 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-[fadeIn_0.3s_ease]"><div className={`${t.card} w-full max-w-sm rounded-t-[2rem] sm:rounded-2xl p-6 border border-white/10 ${t.text} shadow-2xl animate-[slideUp_0.4s_cubic-bezier(0.32,0.72,0,1)]`}>
        <h2 className="font-bold text-lg text-center mb-4">New Post</h2>
        <div className="flex gap-3 mb-4 justify-center">{CATS.map(c=><button key={c} onClick={()=>setCat(c)} className={`px-5 py-2.5 rounded-full text-sm font-black transition-all duration-200 ${cat===c? t.btn : t.sub}`}>{c}</button>)}</div>
        ...
        <div className="flex items-center gap-3 mt-4">
          <button onClick={()=>fileRef.current?.click()} className={`w-12 h-12 ${t.sub} rounded-full flex items-center justify-center text-2xl font-black hover:scale-110 transition-transform`}>+</button>
        ...
        {photo&&<img src={photo} className="mt-4 rounded-xl w-full animate-[fadeIn_0.3s_ease]"/>}
        <div className="flex gap-3 mt-6"><button onClick={()=>setShowPost(false)} className={`flex-1 py-3 rounded-full ${t.sub} font-bold`}>Cancel</button><button onClick={doPost} className={`flex-1 py-3 rounded-full ${t.btn} font-black hover:shadow-lg transition-all`}>Post to {reach}</button></div>
      </div></div>}
      ...
      {showDM&&<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-20 flex items-end sm:items-center justify-center p-0 sm:p-4"><div className={`${t.card} w-full max-w-sm rounded-t-[2rem] sm:rounded-2xl p-6 border border-white/10 max-h-[85vh] overflow-auto ${t.text} shadow-2xl`}>
        ...
        <button onClick={sendDM} className={`w-full ${t.btn} py-4 rounded-full font-black text-base mb-6 hover:shadow-lg transition-all`}>Send DM</button>
        <div className="text-xs opacity-50 mb-2">Recent — tap to reply</div>
        <div className="space-y-2">{dms.map((m:any)=><div key={m.id} onClick={()=>{setDmTo(m.from_user===profile?.full_name? m.to_user : m.from_user);}} className={`${t.sub} p-3 rounded-xl text-sm cursor-pointer hover:scale-[1.01] transition-all`}><b>{m.from_user} → {m.to_user}</b><div className="mt-1 opacity-80">{m.message||m.body}</div></div>)}</div>
