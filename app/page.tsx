...
const KC_ZIPS_5MI = ['64155','64156','64119','64158','64068'];
const KC_ZIPS_40MI = ['64155','64156','64119','64116','64117','64118','64112','64113','64114','64110','64111','64068','64030','64090','64132','64133','64151','64152','64153','64154','64158','64157','64089','64012','64014','64015','64016','64024','64048','64052','64055','64056','64064','64081','64082','64101','64102','64105','64106','64108','64109','64120','64121','64124','64126','64127','64128','64130','64131','64145','64146'];
...
export default function Page(){
  ...
  const handleAiVerify = async()=>{
    ...
    try{
      ...
      if(!res.ok) throw new Error(j.error||'AI failed to read address');
      
      const extractedStreet = j.street||'';
      const extractedZip = j.zip||'';
      const fullAddr = j.full_address||j.full||`${extractedStreet} ${extractedZip}`;
      
      setAiParsedAddress({ street: extractedStreet, zip: extractedZip, city: j.city||'', full: fullAddr });
      ...
      // No duplicate - verified
      setAiVerified(true);
      setAddr(extractedStreet);
      setZip(extractedZip||'64155');
    }catch(e:any){
      ...
      if(type==='mail'){
        ...
        try{
          await supabase.from('verified_addresses').insert({
            street: aiParsedAddress.street,
            zip: aiParsedAddress.zip,
            full_address: aiParsedAddress.full||aiParsedAddress.street,
            owner_name: name,
            verified_at: new Date().toISOString()
          } as any);
        }catch{}
        const pr={full_name:name,email,street_address:aiParsedAddress.street,zip:aiParsedAddress.zip,neighborhood_id:cur?.id, tier:'40mile', is_verified:true, via_mail:true};
        localStorage.setItem('nkc_profile_tiered_40',JSON.stringify(pr));
        localStorage.setItem('nkc_profile',JSON.stringify(pr));
        setProfile(pr);
        setShowJoin(false);
        alert('✅ Verified! 40 mile access unlocked via mail + Bluetooth ready');
      } else {
        const pr={full_name:name,email,street_address:addr,zip:zip||cur?.zip,neighborhood_id:cur?.id, tier:'5mile', is_verified:false};
        localStorage.setItem('nkc_profile_tiered_40',JSON.stringify(pr));
        localStorage.setItem('nkc_profile',JSON.stringify(pr));
        setProfile(pr);
        setShowJoin(false);
  ...
  const deletePost = async(id:string, img:string|null)=>{
    if(!confirm('Delete?')) return;
    try{ await supabase.from('posts').delete().eq('id', id); setPosts(posts.filter((p:any)=>p.id!==id)); if(img){ try{ const path=img.split('/').pop(); if(path) await supabase.storage.from('post-images').remove([path]); }catch{} } }catch(e:any){ alert(e.message); }
  };
  const deleteComment = async(cid:string,pid:string)=>{ try{ await supabase.from('comments').delete().eq('id', cid); setComments((prev)=>{ const next={...prev}; next[pid]=prev[pid].filter((c:any)=>c.id!==cid); return next; }); }catch(e:any){ alert(e.message); } };
    ...
    <div className="min-h-screen bg-[#f8f5ee]">
      <header className="bg-white border-b sticky top-0 z-20 px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex gap-2 items-center">
          {profile ? <><span className="text-xs font-bold">{profile.full_name}</span><Link href="/dms" className="bg-black text-white px-3 py-1.5 rounded-full text-xs">DMs</Link></> : <button onClick={()=>setShowJoin(true)} className="bg-[#1a3a2f] text-white px-4 py-1.5 rounded-full text-sm font-bold">Join</button>}
        </div>
      </header>

      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 grid grid-cols-1 lg:grid-cols-[220px_1fr_300px] gap-6">
        ...
        <main className="space-y-3">
          <div className="bg-white rounded-2xl p-4 border">
            <textarea value={body} onChange={e=>setBody(e.target.value)} placeholder={profile?`What's up in ${cur?.name}?`:'Join Parkwood Hills to post...'} className="w-full bg-[#f8f5ee] rounded-xl p-3 min-h-[80px] text-sm outline-none" />
              ...
              {isOpen && (
                <div className="mt-3 bg-[#f8f5ee] rounded-xl p-3 space-y-2">
                  {cList.map((c:any)=>{
                    ...
                    return (
                      <div key={c.id} className="text-sm bg-white rounded-lg p-2 flex justify-between">
                        <div><b className="text-xs">{c.author_name}:</b> {c.content||c.body} <button onClick={()=>toggleCommentLike(c.id)} className={`ml-2 text-xs ${cliked?'text-red-600':'opacity-50'}`}>{cliked?'❤️':'🤍'} {cl.length}</button></div>
                      </div>
                    );
                  })}
        ...
        <aside className="bg-white rounded-2xl p-5 border h-fit"><h3 className="font-black">{cur?.name}</h3><p className="text-xs opacity-60">{cur?.zip} · KC, MO</p><div className="grid grid-cols-2 gap-2 mt-4"><div className="bg-[#f8f5ee] rounded-xl p-3 text-center"><b className="text-lg">{cur?.member_count||247}</b><p className="text-xs">NEIGHBORS</p></div><div className="bg-[#f8f5ee] rounded-xl p-3 text-center"><b className="text-lg">{posts.length}</b><p className="text-xs">POSTS</p></div></div>{profile?.tier==='40mile' && <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-xl"><p className="text-xs font-black text-green-800">✅ 40 Mile Verified</p><p className="text-[11px] text-green-700 mt-1">Mail + Bluetooth ready</p></div>}</aside>
      </div>

      {showJoin && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white rounded-t-[24px] sm:rounded-[20px] w-full max-w-[480px] p-5 sm:p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4"><h2 className="font-black text-xl">Join {cur?.name}</h2><button onClick={()=>setShowJoin(false)} className="w-8 h-8 rounded-full bg-black/5 font-black">✕</button></div>
            
            <div className="space-y-3">
              <input required value={name} onChange={e=>setName(e.target.value)} placeholder="Full name" className="w-full bg-[#f8f5ee] border rounded-xl px-3 py-3 text-sm outline-none"/>
              <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email (optional)" className="w-full bg-[#f8f5ee] border rounded-xl px-3 py-3 text-sm outline-none"/>
              
              <div className="bg-[#f8f5ee] rounded-xl p-3 border">
                <p className="font-black text-sm">Option 1: 5 Mile (ZIP only)</p>
                <div className="flex gap-2 mt-2">
                  <input value={addr} onChange={e=>setAddr(e.target.value)} placeholder="Street (optional)" className="flex-1 bg-white border rounded-xl px-3 py-2.5 text-sm outline-none"/>
                  <input value={zip} onChange={e=>setZip(e.target.value)} placeholder="ZIP" className="w-[90px] bg-white border rounded-xl px-3 py-2.5 text-sm outline-none"/>
                </div>
              </div>

              <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-3">
                <p className="font-black text-sm text-blue-900">Option 2: 40 Mile via Mail + Bluetooth 📬📲</p>
                <p className="text-[11px] text-blue-700 mt-1">Upload envelope/bill. AI extracts address, checks duplicate, unlocks 40mi. Owner can add you via Bluetooth tap if address taken.</p>
                <input type="file" accept="image/*" onChange={e=>setMailFile(e.target.files?.[0]||null)} className="mt-2 text-xs w-full"/>
                <button disabled={!mailFile||aiVerifying} onClick={handleAiVerify} className="mt-2 w-full bg-blue-600 text-white py-2.5 rounded-full font-black text-sm disabled:opacity-30">
                  {aiVerifying ? '🤖 AI Reading...' : '🤖 Verify Mail with AI'}
                </button>
                {aiParsedAddress && <div className="mt-2 p-2.5 bg-green-50 border border-green-300 rounded-xl text-[11px]"><p className="font-black text-green-800">✓ AI Extracted:</p><p className="text-green-700">{aiParsedAddress.street} | {aiParsedAddress.zip} {aiParsedAddress.city}</p></div>}
                {verifyError && <div className="mt-2 p-2.5 rounded-xl text-[11px] font-bold bg-red-50 border border-red-300 text-red-700">{verifyError}</div>}
                {showBluetoothRequest && (
                  <div className="mt-3 p-3 bg-blue-50 border-2 border-blue-300 rounded-xl">
                    ...
                    <p className="text-[11px] text-blue-800 mt-1">Is {showBluetoothRequest.owner} nearby? Ask them to approve you.</p>
                    <button onClick={async()=>{
                      try{
                        ...
                        alert(`Request sent to ${showBluetoothRequest.owner}! Ask them to check DMs.`);
                      }catch(e:any){ alert('Failed: '+e.message); } finally{ setBluetoothScanning(false); }
                    }} className="w-full mt-2 bg-blue-600 text-white py-2.5 rounded-full font-black text-xs">
                      {bluetoothScanning ? 'Sending...' : '📲 Request Bluetooth Tap'}
                    </button>
                  </div>
                )}
              </div>
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button disabled={!aiVerified} onClick={()=>handleJoin('mail')} className="flex-1 py-3 rounded-full font-bold text-sm bg-black text-white disabled:opacity-30">40 Mile 🤖</button>
            </div>
            {aiVerified && <p className="text-[11px] text-green-600 text-center mt-2 font-bold">✓ Verified & ready to join with 40 mile + Bluetooth</p>}
          </div>
        </div>
      )}
    </div>
