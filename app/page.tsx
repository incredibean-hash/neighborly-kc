...
export default function Page(){
  ...
  const handleAiVerify = async()=>{
    ...
    try{
      ...
      if(!res.ok) throw new Error(j.error||'AI failed - '+JSON.stringify(j));
      const fullAddr = j.full_address||j.full||`${j.street} ${j.zip}`;
      setAiParsedAddress({ street: j.street, zip: j.zip, city: j.city||'', full: fullAddr });
      try{
  ...
  return (
    <div className="min-h-screen bg-[#f8f5ee]">
      <header className="bg-white border-b sticky top-0 z-20 px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex gap-2">{profile?<><span className="text-xs">{profile.full_name}</span><Link href="/dms" className="bg-black text-white px-3 py-1 rounded-full text-xs">DMs</Link></>:<button onClick={()=>setShowJoin(true)} className="bg-[#1a3a2f] text-white px-4 py-1.5 rounded-full text-sm font-bold">Join</button>}</div>
      </header>
          ...
          <div className="bg-white rounded-t-[24px] sm:rounded-[20px] w-full max-w-[480px] p-5 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-3">
                ...
                <p className="text-[11px] text-blue-700 mt-1">Upload envelope/bill. AI extracts address (handles upside-down), checks duplicate, unlocks 40mi.</p>
                <input type="file" accept="image/*" onChange={e=>setMailFile(e.target.files?.[0]||null)} className="mt-2 text-xs w-full"/>
                <button disabled={!mailFile||aiVerifying} onClick={handleAiVerify} className="mt-2 w-full bg-blue-600 text-white py-2.5 rounded-full font-black text-sm disabled:opacity-30">{aiVerifying?'🤖 AI Reading (handles upside-down)...':'🤖 Verify Mail with AI'}</button>
                {aiParsedAddress && <div className="mt-2 p-2.5 bg-green-50 border border-green-300 rounded-xl text-[11px]"><p className="font-black text-green-800">✓ AI Extracted:</p><p className="text-green-700">{aiParsedAddress.street} | {aiParsedAddress.zip} | {aiParsedAddress.full}</p></div>}
