...
const MAX_POST_SIZE = 3 * 1024 * 1024; // 3MB
...
export default function Page(){
  ...
  const [verifyFile,setVerifyFile]=useState<File|null>(null); const [verifying,setVerifying]=useState(false); const [verifyStatus,setVerifyStatus]=useState<string>('');
  ...
  useEffect(()=>{ if(!activeThread) return; const int=setInterval(async ()=>{ const {data:msgs}=await supabase.from('dm_messages').select('*').eq('thread_id', activeThread.id).order('created_at',{ascending:true}); if(msgs) setDmMessages(msgs); }, 2000); return ()=>clearInterval(int); },[activeThread]);
  ...
  const validatePostFile = async (f:File): Promise<string|null> => {
    if(!ALLOWED_TYPES.includes(f.type)) return 'Only JPG/PNG photos. No GIFs, no memes.';
    if(f.size > MAX_POST_SIZE) return `File too big: ${(f.size/1024/1024).toFixed(1)}MB. Max 3MB.`;
    return null;
  };

  const handlePost = async () => {
    if(!profile) return setShowJoin(true); if(!body.trim() &&!file) return;
    if(file){ const err=await validatePostFile(file); if(err){ alert(err); return; } }
    setUploading(true);
    try{
  ...
  const deletePost = async (id:string, image_url:string|null) => { if(!confirm('Delete this post?')) return; await supabase.from('posts').delete().eq('id', id); if(image_url){ const path=image_url.split('/post-images/')[1]; if(path) await supabase.storage.from('post-images').remove([path]); } setPosts(posts.filter((p:any)=>p.id!==id)); };
  const deleteComment = async (cId:string, postId:string) => { if(!confirm('Delete comment?')) return; await supabase.from('comments').delete().eq('id', cId); setComments((prev)=>{ const next={...prev}; next[postId]=prev[postId].filter((x:any)=>x.id!==cId); return next; }); };

  const startDM = async (otherName:string) => {
    if(!profile) return setShowJoin(true); const cleanName = (otherName||'').trim(); if(!cleanName || cleanName==='Neighbor') return alert('That user has no name yet'); if(cleanName===profile.full_name) return alert("That's you!");
    const [a,b]=[profile.full_name, cleanName].sort(); const {data:found}=await supabase.from('dm_threads').select('*').eq('user_a',a).eq('user_b',b).maybeSingle(); let existing=found;
    if(!existing){ const {data:newThread}=await supabase.from('dm_threads').insert({user_a:a, user_b:b, last_message:'Chat started', last_message_at: new Date().toISOString()}).select().single(); existing=newThread; }
    setActiveThread(existing); setShowDM(true); await supabase.from('dm_messages').update({status:'delivered'}).eq('thread_id', existing.id).neq('sender_name', profile.full_name).eq('status','sent');
    const {data:msgs}=await supabase.from('dm_messages').select('*').eq('thread_id', existing.id).order('created_at',{ascending:true}); setDmMessages(msgs||[]); loadThreads(); setTimeout(()=>dmInputRef.current?.focus(),400);
  };
  const openThread = async (thread:any) => {
    setActiveThread(thread); if(profile){ await supabase.from('dm_messages').update({status:'read', read_at: new Date().toISOString()}).eq('thread_id', thread.id).neq('sender_name', profile.full_name).in('status',['sent','delivered']); }
    const {data:msgs}=await supabase.from('dm_messages').select('*').eq('thread_id', thread.id).order('created_at',{ascending:true}); setDmMessages(msgs||[]); setTimeout(()=>dmInputRef.current?.focus(),300);
  };
  const sendDM = async () => {
    if(!dmInput.trim() || !activeThread || !profile) return;
    const {data}=await supabase.from('dm_messages').insert({thread_id:activeThread.id, sender_name:profile.full_name, body:dmInput.trim(), status:'sent'}).select().single();
    if(data){ setDmMessages([...dmMessages, data]); await supabase.from('dm_threads').update({last_message:dmInput.trim().slice(0,40), last_message_at: new Date().toISOString()}).eq('id', activeThread.id); setDmInput(''); loadThreads(); setTimeout(()=>dmInputRef.current?.focus(),100); }
  };

  const handleVerifiedJoin = async (e:any) => {
    e.preventDefault();
    if(!name.trim()) return alert('Enter name');
    setVerifying(true); setVerifyStatus('Uploading proof...');
    try{
      let verificationApproved = true; // default if no file
      let imagePath = null;
      if(verifyFile){
        // validate mail file
        if(!ALLOWED_TYPES.includes(verifyFile.type)) throw new Error('Mail must be JPG/PNG');
        if(verifyFile.size > 3*1024*1024) throw new Error('Mail photo max 3MB');
        const path = `${name.replace(/\s+/g,'_')}-${Date.now()}.jpg`;
        const compressed = await compressImage(verifyFile);
        const {error: upErr} = await supabase.storage.from('verification-docs').upload(path, compressed);
        if(upErr) throw upErr;
        imagePath = path;
        setVerifyStatus('AI checking address...');
        // Create signed URL for AI to read
        const {data: signed} = await supabase.storage.from('verification-docs').createSignedUrl(path, 60);
        if(signed?.signedUrl){
          try{
            const res = await fetch('/api/verify-address', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({imageUrl: signed.signedUrl, claimedAddress: addr, claimedZip: cur?.zip})});
            const result = await res.json();
            verificationApproved = result.approved;
            setVerifyStatus(result.approved ? '✓ Verified!' : 'AI says: '+result.reason);
            await supabase.from('verification_requests').insert({full_name:name, claimed_address:addr, claimed_zip:cur?.zip, email, image_path:path, status: result.approved ? 'approved':'pending', ai_reason: result.reason, ai_confidence: result.confidence || 0.5});
            if(!result.approved){
              // Still allow join but not verified badge
              await new Promise(r=>setTimeout(r,2000));
            }
          }catch(err){ console.log('AI verify failed, manual review', err); await supabase.from('verification_requests').insert({full_name:name, claimed_address:addr, claimed_zip:cur?.zip, email, image_path:path, status:'pending', ai_reason:'AI API not configured yet'}); }
        }
      } else {
        // No file - still allow but unverified
        verificationApproved = false;
      }
      const pr={full_name:name,email,street_address:addr,zip:cur?.zip,neighborhood_id:cur?.id, verified: verificationApproved, verification_path: imagePath};
      localStorage.setItem('nkc_profile',JSON.stringify(pr)); setProfile(pr); setShowJoin(false); setVerifyFile(null); setVerifyStatus('');
      if(verificationApproved && imagePath) alert('✓ Verified neighbor! Badge added.');
    }catch(err:any){ alert('Verification failed: '+err.message); } finally{ setVerifying(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8f5ee] text-[#1a3a2f] w-full overflow-x-hidden">
      <header className="sticky top-0 bg-white border-b z-40 w-full">
        <div className="max-w-6xl mx-auto px-3 sm:px-6 h-auto sm:h-14 py-3 sm:py-0 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="flex gap-2 w-full sm:w-auto items-center flex-wrap">
            ...
            {profile?<span className="bg-[#1a3a2f] text-white px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm shrink-0">{profile.verified ? '✓ ' : ''}Hi, {profile.full_name.split(' ')[0]}</span>:<button onClick={()=>setShowJoin(true)} className="bg-[#1a3a2f] text-white px-4 sm:px-5 py-2 rounded-full text-xs sm:text-sm font-bold shrink-0">Join {cur?.name}</button>}
          </div>
        </div>
      </header>

      <div className="w-full flex justify-center"><div className="w-full max-w-6xl px-3 sm:px-6 py-6 sm:py-10">
        <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr_300px] gap-4 sm:gap-6 w-full justify-center">
          <aside className="lg:sticky lg:top-24 h-fit w-full max-w-full overflow-hidden lg:mt-8">
            <div className="lg:hidden mb-3"><button onClick={()=>setShowFilters(!showFilters)} className="w-full bg-white border rounded-xl px-4 py-3 text-sm font-bold flex justify-between items-center shadow-sm">Filter: {cat} <span>{showFilters?'▲':'▼'}</span></button>{showFilters && <div className="mt-2 bg-white rounded-xl border p-2 grid grid-cols-2 gap-2 shadow-sm">{CATS.map(c=><button key={c} onClick={()=>{setCat(c); setShowFilters(false);}} className={`px-3 py-2.5 rounded-xl text-xs font-bold text-left truncate ${cat===c?'bg-[#1a3a2f] text-white':'bg-[#f8f5ee]'}`}>{c}</button>)}</div>}</div>
          ...
          <main className="w-full max-w-[680px] mx-auto min-w-0 space-y-4 mt-2 sm:mt-8">
            <div className="bg-white rounded-2xl p-3 sm:p-4 border w-full shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-3 w-full"><input id="file-input" type="file" accept="image/jpeg,image/png,image/jpg" onChange={async e=>{ const f=e.target.files?.[0]; if(!f) return; const err=await validatePostFile(f); if(err){ alert(err); e.target.value=''; return; } setFile(f); }} className="text-xs w-full sm:w-auto max-w-full truncate" />{file && <span className="text-[10px] opacity-60 truncate">{(file.size/1024).toFixed(0)}KB → ~400KB ✓</span>}</div>
            ...
            {filtered.map((p:any)=>{
              ...
              const isOwner = profile && (p.profiles?.full_name===profile.full_name || p.author_name===profile.full_name); const canDelete = isOwner || isAdmin; const authorName = p.author_name || p.profiles?.full_name || 'Neighbor';
              return (
              <div key={p.id} className="bg-white rounded-2xl p-3 sm:p-4 border w-full shadow-sm min-w-0 overflow-hidden">
                <div className="flex justify-between gap-2 min-w-0"><button onClick={()=>startDM(authorName)} className="text-xs font-bold opacity-80 truncate min-w-0 flex-1 text-left hover:underline">👤 {authorName} {p.author_name && profile?.verified ? '✓' : ''} · {p.category} → Message</button>{canDelete && <button onClick={()=>deletePost(p.id,p.image_url)} className="text-xs opacity-40 hover:text-red-600 shrink-0">🗑️ Delete</button>}</div>
                ...
                <div className="mt-3 pt-3 border-t flex gap-4">
                  ...
                  <button onClick={()=>setOpenComments((prev)=>({...prev,[p.id]:!prev[p.id]}))} className="text-xs font-bold hover:underline opacity-60">💬 {cList.length} {isOpen?'▲':'▼'}</button>
      ...
      {showJoin && <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl w-full max-w-sm p-6 max-h-[90vh] overflow-y-auto">
        <h2 className="font-black text-xl">Join {cur?.name}</h2><p className="text-xs opacity-60 mt-1">Get verified with mail photo (optional but gets ✓ badge)</p>
        <form onSubmit={handleVerifiedJoin} className="mt-4 space-y-3">
          <input required value={name} onChange={e=>setName(e.target.value)} placeholder="Full name" className="w-full bg-[#f8f5ee] border rounded-xl px-3 py-3 text-sm"/>
          <input required value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email" className="w-full bg-[#f8f5ee] border rounded-xl px-3 py-3 text-sm"/>
          <input required value={addr} onChange={e=>setAddr(e.target.value)} placeholder={`Address in ${cur?.zip} (e.g. 1234 NW 80th St)`} className="w-full bg-[#f8f5ee] border rounded-xl px-3 py-3 text-sm"/>
          <div className="bg-[#f8f5ee] rounded-xl p-3 border-2 border-dashed">
            <p className="text-xs font-bold">📬 Verify Address (optional)</p><p className="text-[10px] opacity-60">Upload mail / package label showing your address. Private bucket, auto-deletes after 24h.</p>
            <input type="file" accept="image/jpeg,image/png,image/jpg" onChange={async e=>{ const f=e.target.files?.[0]; if(!f) return; if(f.size>3*1024*1024){ alert('Max 3MB for mail photo'); e.target.value=''; return; } if(!ALLOWED_TYPES.includes(f.type)){ alert('Only JPG/PNG'); e.target.value=''; return; } setVerifyFile(f); }} className="w-full text-xs mt-2" />
            {verifyFile && <p className="text-[10px] text-green-600 mt-1">✓ {verifyFile.name} ({(verifyFile.size/1024).toFixed(0)}KB) ready</p>}
            {verifyStatus && <p className="text-xs font-bold mt-2 text-[#1a3a2f]">{verifyStatus}</p>}
          </div>
          <div className="flex gap-2 pt-2"><button type="button" onClick={()=>setShowJoin(false)} className="flex-1 bg-[#f8f5ee] py-3 rounded-full font-bold text-sm">Cancel</button><button disabled={verifying} className="flex-1 bg-[#1a3a2f] text-white py-3 rounded-full font-bold text-sm disabled:opacity-50">{verifying ? 'Verifying...' : 'Join Parkwood Hills'}</button></div>
        </form>
      </div></div>}

      {showDM && (
        <div className="fixed inset-0 z-[60] flex justify-end"><div className="absolute inset-0 bg-black/30" onClick={()=>setShowDM(false)}></div>
          <div className="relative w-full sm:w-[400px] bg-white h-full flex flex-col shadow-2xl">
            <div className="p-4 border-b flex justify-between items-center bg-[#1a3a2f] text-white"><b>💬 Messages • ✓✓ = read</b><button onClick={()=>setShowDM(false)} className="opacity-70 hover:opacity-100">✕</button></div>
            {!activeThread ? (
              <div className="flex-1 overflow-y-auto"><p className="p-4 text-xs font-bold opacity-40">YOUR CONVERSATIONS</p>{threads.length===0 && <p className="p-4 text-sm opacity-60">No DMs yet. Click DM on any post!</p>}{threads.map((t:any)=>(<button key={t.id} onClick={()=>openThread(t)} className="w-full text-left p-4 border-b hover:bg-[#f8f5ee] flex justify-between items-center"><div className="min-w-0 flex-1"><p className="font-bold text-sm truncate">{t.user_a===profile?.full_name ? t.user_b : t.user_a}</p><p className="text-xs opacity-60 truncate">{t.last_message}</p></div><span className="text-[10px] opacity-40 shrink-0 ml-2">{new Date(t.last_message_at).toLocaleDateString()}</span></button>))}</div>
            ) : (
              <div className="flex-1 flex flex-col min-h-0">
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-[#f8f5ee]">
                  {dmMessages.map((m:any)=>{ const isMe = m.sender_name===profile?.full_name; const status=m.status||'sent'; return (<div key={m.id} className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm break-words ${isMe?'bg-[#1a3a2f] text-white ml-auto rounded-br-sm':'bg-white border mr-auto rounded-bl-sm'}`}><p>{m.body}</p><div className={`flex items-center gap-1 justify-end mt-1`}><p className={`text-[9px] ${isMe?'opacity-60':'opacity-40'}`}>{new Date(m.created_at).toLocaleTimeString([],{hour:'2-digit', minute:'2-digit'})}</p>{isMe && <span className={`text-[11px] font-bold ${status==='read'?'text-[#53bdeb]':'text-white/60'}`}>{status==='sent'?'✓':status==='delivered'?'✓✓':'✓✓'}</span>}</div></div>); })}
                  <div ref={dmBottomRef}></div>
                </div>
                <div className="p-3 border-t flex gap-2"><input ref={dmInputRef} value={dmInput} onChange={e=>setDmInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter') sendDM();}} placeholder="Message..." className="flex-1 bg-[#f8f5ee] rounded-full px-4 py-3 text-sm outline-none min-w-0" autoFocus /><button onClick={sendDM} className="bg-[#1a3a2f] text-white px-5 py-3 rounded-full text-sm font-bold shrink-0">Send</button></div>
              </div>
            )}
          </div>
        </div>
