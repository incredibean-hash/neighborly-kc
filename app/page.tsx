...
const RADIUS_OPTIONS = [
  {id:'hood', label:'My Neighborhood Only'},
  {id:'5', label:'5 Mile Radius'},
  {id:'10', label:'10 Mile Radius'},
  {id:'25', label:'25 Mile Radius - RECOMMENDED'},
  {id:'metro', label:'KC Metro'},
];
...
export default function Page(){
  ...
  const handleJoin = ()=>{
    ...
    const pr = { full_name:name.trim(), street_address:addr.trim(), zip:cleanZip, verified:true };
  ...
  const addPost = async ()=>{
    ...
    const finalContent = `[${reach}] ${text}`;
    const { error } = await supabase.from('posts').insert({ user_name: profile.full_name, content: finalContent, category: cat });
      ...
      <div className="min-h-screen flex items-center justify-center p-6 bg-[#f8f5ee]">
        <div className="bg-white p-8 rounded-[28px] shadow-xl w-full max-w-lg border-2">
          ...
          <div className="space-y-3">
            ...
            <div className="text-xs font-bold opacity-60 p-2">Covers 25 miles: Gladstone, Liberty, Briarcliff, Overland Park, Lees Summit, etc.</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[200px_1fr_360px] gap-6">
          <div className="space-y-3">
            ...
            <div className="bg-[#1a3d2e] text-white rounded-2xl p-4 border-2">
              ...
              <p className="text-xs opacity-80 mt-1">From Parkwood Hills - Covers all KC</p>
            </div>
          </div>
            ...
            <div className="bg-white rounded-[24px] p-5 shadow-sm border-2 border-black/5">
              ...
              <div className="mt-3 space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold opacity-60">Will reach 25 miles</span>
                  <button onClick={addPost} className="bg-black text-white px-8 py-3 rounded-full font-black text-lg">Post to 25 Miles</button>
                </div>
              </div>
            </div>
            <div className="space-y-3">
              {filtered.map((p:any)=>(
                <div key={p.id} className="bg-white rounded-[24px] p-6 shadow-sm border-2 border-black/5">
                  <div className="flex justify-between items-center">
                    <span className="font-black text-lg">{p.user_name} ✓</span>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-[24px] p-6 shadow-sm border-2 border-black/5">
              ...
              <p className="text-sm font-bold opacity-60 mb-4">25 Mile Radius • Verified</p>
              <div className="border-t-2 pt-5">
                ...
                <input value={dmMsg} onChange={e=>setDmMsg(e.target.value)} placeholder="Message" className="w-full border-2 border-black/10 p-3.5 rounded-xl mb-3 text-sm"/>
