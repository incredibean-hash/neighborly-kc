...
export async function POST(req: NextRequest){
  try{
    ...
    const { data: approval } = await supabase.from('bluetooth_approvals').select('*').eq('token', token).single();
    if(!approval) return NextResponse.json({ success:false, error:'Invalid token' }, {status:404});
    if((approval as any).status !== 'pending') return NextResponse.json({ success:false, error:'Already used' }, {status:400});
    const created = new Date((approval as any).created_at).getTime();
    if(Date.now() - created > 15*60*1000) return NextResponse.json({ success:false, error:'Expired' }, {status:400});
